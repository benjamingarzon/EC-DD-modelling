...
transformed parameters {
  ...
  drift = mu_p[2] + sigma_p[2] * drift_intercept_pr[instance] + (mu_slope_p + sigma_slope_p * drift_slope_pr[instance]).* ev;
...
model {
  // Hyperparameters
  mu_p  ~ normal(0, 10);        
  sigma_p ~ cauchy(0, 5); #normal(0, 10); #
  mu_slope_p ~ normal(0, 2);        
  sigma_slope_p ~ cauchy(0, 2); #normal(0, 2); #~ cauchy(0, 2);
