...
choicedata.common = choicedata.common %>% mutate(Context = ifelse(context == 'c1', 'High volatility', 'Low volatility'),
                                   Choice = ifelse(choice == 1, 'Delayed option', 'Immediate option'))
...
myplot.choiceprob = ggplot(data = choicedata.group, aes(x = ev, y = prob_late.mean, ymin = prob_late.mean - prob_late.sem, 
  ...
  geom_errorbar() + 
  xlab('Later amount') + 
  ylab('Probability of delayed option') + theme(legend.position = 'bottom', legend.title = element_blank() ) 
...
myplot.reliab = ggplot(allIndPars.melt, aes(x = Context, y = value, group = subjID)) + geom_line() + facet_wrap( .~ parameter_label, scales = "free", nrow = 1 )
print(myplot.reliab)
...
myplot.differences = ggplot(allIndPars.sum, aes(x=Context, y=value.corr)) + 
...
fig.a = ggarrange(myplot.RT, myplot.RT.choice, myplot.choiceprob, myplot.range, ncol=2, nrow=2)
ggsave(filename=file.path(FIGS_DIR, "Choices.png"), plot = fig.a, dpi = DPI, width = WIDTH2COL, height = HEIGHT2ROW)

fig.b = ggarrange(myplot.reliab, myplot.differences, ncol=1, nrow=2)
ggsave(filename=file.path(FIGS_DIR, "ChoiceParameters.png"), plot = fig.b, dpi = DPI, width = WIDTH2COL, height = HEIGHT2ROW)
