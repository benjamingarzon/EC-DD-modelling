...
choicedata.common = choicedata.common %>% mutate(
  Context = ifelse(context == 'c1', 'High variance', 'Low variance'),
...
# Distribution of delay later
```{r}
cdelay = choicedata.common %>% filter(subjID %in% include_subjects) %>% group_by(subjID) %>% summarise(delay_later = first(delay_later))

print(mean(cdelay$delay_later))
print(sd(cdelay$delay_later))
print(range(cdelay$delay_later))

ggplot(cdelay, aes(x = delay_later)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Delay Later", x = "Delay (later)", y = "Frequency") +
  theme_minimal() +
...
for (co in c("High var. first", "Low var. first")) {
for (mypar in mypars) {
...
myplot = ggplot(allIndPars.sum, aes(x=parameter, y=value)) + 
    ...
    geom_line() +
    geom_point() + 
    ylab('Parameter difference (low variance - high variance)') + 
...
mydata = mydata %>% mutate(
  ...
  Group_tag = ifelse(Group == 'High var. first', 'HF', 'LF')
)
...
fileout = sprintf('../results/results_text_fixed_%f.txt', PRIOR_SD)
