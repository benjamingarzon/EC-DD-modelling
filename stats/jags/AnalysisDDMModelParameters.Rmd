...
choicedata = merge(choicedata.common, allIndPars %>% filter(subjID %in% include_subjects), by = c('subjID', 'Context', 'context', 'context_order', 'background_col'))
choicedata = choicedata %>% mutate( ev = amount_later, context_num = as.numeric(substr(context, 2, 2))) %>% mutate(ev.cut = cut(ev, breaks))

myplot.RT = ggplot(data = choicedata, aes(x = ev, y = rt, color = Context, group = subjID)) + geom_line(size=0.2) + 
  geom_point(size=0.2) + 
  xlab('Later amount ($)') +
...
choicedata.group = choicedata %>% 
  group_by(subjID, Context, context_order, ev.cut) %>% 
...
model = lmer(log(rt) ~ context_order*context*(amount_later_centered + amount_later_centered.2) + (1 + amount_later_centered|subjID), data = choicedata)
print(summary(model))
```
...
ratingdata = merge(ratingdata.sub.clean, allIndPars %>% filter(subjID %in% include_subjects), by = c('subjID', 'context', 'context_num', 'context_order', 'background_col', 'Context'))
...
ratingdata.group = ratingdata %>% 
  group_by(Context, context_order, amount_later) %>% 
  summarise(key.rt.mean = mean(key.rt)/1000,key.rt.sem = sem(key.rt)/1000, ev = mean(ev))
...
allIndPars.sum <- summarySE(allIndPars.melt, measurevar="value.corr", groupvars=c("Context","parameter_label", "context_order"))
