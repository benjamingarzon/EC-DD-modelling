...
myplot.RT.choice = ggplot(data = choicedata.group.choice, aes(x = ev, y = rt.mean, ymin = rt.mean - rt.sem, ymax = rt.mean + rt.sem, color = Context, group = Context)) +
  ...
  geom_errorbar() + 
  xlab('Later amount ($)') + 
  ylab('Response time (s)') + facet_grid( .~Choice ) + theme(legend.position = 'bottom', legend.title = element_blank() ) 
...
model = lmer(log(rt) ~ context*(amount_later_centered + amount_later_centered.2) + choice + context_order + (1 + amount_later_centered|subjID), data = choicedata)
print(summary(model))
