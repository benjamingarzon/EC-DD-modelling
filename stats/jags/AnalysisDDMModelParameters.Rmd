...
for (mypar in mypars){
  for (co in c(1, 2)){
     myformula = as.formula(sprintf("%s ~ scale(%s)*scale(amount_later) + (1| subjID)", mypar.rating, mypar)) 
...
myplot.rating.pars = ggplot(data = ratingdata.subj, aes_string(
  y = mypar.rating,
  x = mypar,
  color = 'Context',
  group = 'subjID'
...
myplot.pairs <- ggpairs(mydata, columns = mypars, ggplot2::aes(colour=GroupContext), 
...
fig.a.1 = ggarrange(myplot.trial.RT,
                   ncol=1, nrow=1, labels = c('A'))
fig.a.2 = ggarrange(myplot.RT.choice.group, myplot.RT.choice.agg,
...
ggsave(filename=file.path(FIGS_DIR, "Associations.png"), plot = fig.d, dpi = DPI, width = WIDTH1COL*5, height = HEIGHT1ROW*2)
