...
for (mypar.rating in mypars.rating){
  for (mypar in mypars){
    ...
    { 
      ...
      myformula.b = as.formula(sprintf("%s ~ mediator*amount_later_centered + Context", mypar.rating, mypar))
    }
    else
    {
    ...
    med = mediate(model.m, model.b, treat = "Context", mediator = "mediator", boot = T, sims = 1000,
...
colnames(acme.mat) = c("WTP parameter", "DDM parameter", "Context order", "ACME Estimate", "ACME  CI 2.5%", "ACME CI 97.5%", "p-value")
colnames(prop.mat) = c("WTP parameter", "DDM parameter", "Context order", "Proportion Estimate", "Proportion CI 2.5%", "Proportion CI 97.5%", "p-value")
...
statslist['_prop_drift_sensitivity1_'] = as.numeric(as.character(mediation.mat[1, 7]))*100
statslist['_prop_drift_sensitivity2_'] = as.numeric(as.character(mediation.mat[6, 7]))*100
statslist['_prop_noise2_'] = as.numeric(as.character(mediation.mat[9, 7]))*100
statslist['_prop_nondectime2_'] = as.numeric(as.character(mediation.mat[10, 7]))*100
...
for (mypar.rating in mypars.rating){
  for (mypar in mypars){
    ...
    { 
      # myformula.b = as.formula(sprintf("%s ~ mediator*amount_later_centered + Context*Group + (1| subjID)", mypar.rating, mypar))
      myformula.b = as.formula(sprintf("%s ~ mediator*amount_later_centered + Context*Group", mypar.rating, mypar))
    }
    else
    {
      # myformula.b = as.formula(sprintf("%s ~ mediator + Context*Group + (1| subjID)", mypar.rating, mypar))
    ...
    med = mediate(model.m, model.b, treat = "Context", mediator = "mediator", boot = T, sims = 500,
                  control.value="Low volatility", treat.value="High volatility")
    #print(paste(mypar.rating, mypar))
    #print(c(med$d0, med$d0.ci, med$d0.p ))
    #print(c(med$tau.coef, med$tau.ci, med$tau.p ))
