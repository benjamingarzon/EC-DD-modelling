...
MYFILE = 'results/attributewise_simple_ddm-03-04-2021_06:08-10000-4e+05-pars.RData'
#MYFILE = 'results/attributewise_exponential_ddm-03-04-2021_05:54-10000-4e+05-pars.RData'
# Load previous fit
load(MYFILE)
...
subjList <- unique(choicedata.common[c('subjID', 'context', 'context_num', 'context_order', 'background_col', grouping_var)])  # list of 
numSubjs = dataList$N
...
for (i in 1:numSubjs) {
  allIndPars[i, ] <- c( 
  mean(parVals$b.drift.delay.p[i, ]), 
  mean(parVals$b.drift.amount.p[i, ]), 
  mean(parVals$bias.p[i, ]), 
  mean(parVals$nondectime.p[i, ]), 
  mean(parVals$noise.p[i, ])#,
#  mean(parVals$gamma.p[i, ]) 
#  mean(parVals$time.p[i, ]) 
  )
}

allIndPars = cbind(allIndPars, subjList[-6]) 
...
plot(allIndPars$upper_bound[order(allIndPars$lower_bound)], type = 'l', ylim = c(-2, 4))
lines(allIndPars$lower_bound[order(allIndPars$lower_bound)]  , type = 'l')
abline(0, 0, col = 'red')
# NEX = 25                                                                                         
...
write.table(include_subjects, file = "results/included_subjects.txt", row.names = F, col.names = F)
...
{
  get_par("b.drift.delay.p", var)
  get_par("b.drift.amount.p", var)
  get_par("bias.p", var )
  get_par("nondectime.p", var)
  get_par("noise.p", var)
...
mypars = c("b.drift.delay.p", "b.drift.amount.p", "bias.p", "nondectime.p", "noise.p") #, "gamma.p")
...
ratingdata = ratingdata %>% mutate( drift = b.drift.amount.p * (amount_later  - 1) + b.drift.delay.p * (delay_later - 0), bid.diff = (bid.high - bid.low)) %>% mutate(drift2 = drift^2)
